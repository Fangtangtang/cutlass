...
struct Recorder {
    ...
    __device__ __forceinline__ bool init() {
        ...
        cur = p.buf + MAX_WARP_PER_BLOCK + warp_id() * p.capacity * RECORD_INTS;
        return true;
    }
...
inline int64_t buf_total_ints(int per_thread_capacity) {
    return MAX_WARP_PER_BLOCK + MAX_WARP_PER_BLOCK * per_thread_capacity * RECORD_INTS;
}
...
inline void teardown(GPUTraceParam& p,
    ...
    for (int t = 0; t < MAX_WARP_PER_BLOCK; ++t) {
        ...
        if (used <= 0)
            continue;
        int64_t warp_base = MAX_WARP_PER_BLOCK + (int64_t)t * p.capacity * RECORD_INTS;
